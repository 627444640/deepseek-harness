...
- 安装器从打包好的 release tarball 暂存 harness runtime 闭包：`scripts/release/stage-desktop-runtime.ts` 把 `--from` 目录下的每个 tarball 作为 `file:` 依赖安装（与 `release:verify-packed-install` 所验证的机制相同），因为 vendored 框架与 Landlock 入口是 registry 无法解析的私有重作用域包。原生模块在应用随附的 Node 系列下安装，因此 CI 用完全相同的系列暂存闭包。
